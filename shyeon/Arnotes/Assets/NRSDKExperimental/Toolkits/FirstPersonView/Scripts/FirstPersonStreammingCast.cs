...
namespace NRKernal.Experimental.StreammingCast
{
    ...
    /// <summary> A first person streaming cast. </summary>
    public class FirstPersonStreammingCast : MonoBehaviour
    {
        public delegate void OnResponse(bool result);
        ...
        /// <summary> Starts streaming immediately on start. </summary>
        private void StartStreamingImmediately()
        {
            OnStream(response => { });
        }
        ...
        /// <summary> Starts video capture for streaming. </summary>
        public IEnumerator StartVideoCapture()
        {
            Resolution cameraResolution = GetResolutionByLevel(m_ResolutionLevel);
            NRDebugger.Info("[FPStreammingCast] cameraResolution:" + cameraResolution);
        ...
        void OnStartedVideoCaptureMode(NRVideoCapture.VideoCaptureResult result)
        {
            if (!result.success)
            {
                NRDebugger.Info("[FPStreammingCast] Started Video Capture Mode Failed!");
                return;
            }
        ...
        /// <summary> Executes the 'started recording video' action for streaming. </summary>
        /// <param name="result"> The result.</param>
        void OnStartedRecordingVideo(NRVideoCapture.VideoCaptureResult result)
        {
            NRDebugger.Info("[FPStreammingCast] Started Recording Video for Streaming!");
        }
